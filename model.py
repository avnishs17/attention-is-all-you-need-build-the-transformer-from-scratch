...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    id_ = []
    for word in sentence.split():
        id_.append(token_to_id.get(word,token_to_id[unk_token]))
    return id_
...
def combine_padding_and_causal_masks(padding_mask, causal_mask):
    ...
    L = padding_mask.shape[-1]

    padding_mask = padding_mask.expand(B, 1, L, L)
    causal_mask = causal_mask.expand(B, 1, L, L)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    return multi_head_tensor.permute(0, 2, 1, 3).reshape(B, L, num_heads * d_k)
